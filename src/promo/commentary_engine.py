...
# Add the missing determine_line_quality function
...
# Define individual comment collections for direct use in the commentary engine
PERFECT_COMMENTS = QUALITY_COMMENTS['perfect']
EXCELLENT_COMMENTS = QUALITY_COMMENTS['excellent']
GOOD_COMMENTS = QUALITY_COMMENTS['good']
NEUTRAL_COMMENTS = QUALITY_COMMENTS['neutral']
BAD_COMMENTS = QUALITY_COMMENTS['bad']
TERRIBLE_COMMENTS = QUALITY_COMMENTS['terrible']
FLOP_COMMENTS = QUALITY_COMMENTS['flop']
...
def generate_commentary(beat):
    """Generate commentary for a given beat based on its properties."""
    ...
    # Handle intro beats
    if beat.get('is_first_beat', False) or beat.get('phase') == 'opening':
        commentary['is_intro'] = True
        return commentary
        
    # Handle summary beats
    if beat.get('is_last_beat', False) or beat.get('phase') == 'ending':
    ...
    if versus_mode:
        commentary['commentary_line'] = generate_versus_commentary(beat, score)
    else:
        commentary['commentary_line'] = generate_standard_commentary(beat, score)
    
    return commentary

def generate_standard_commentary(beat, score):
    """Generate commentary for standard (single wrestler) promos."""
    phase = beat.get('phase', 'middle')
    
    # Commentary pool by score range and phase
    commentary_pool = {
        'opening': {
            'great': [
    ...
    if phase in commentary_pool and quality in commentary_pool[phase]:
        return random.choice(commentary_pool[phase][quality])
    else:
        # Fallback to middle phase if the specified phase isn't found
        return random.choice(commentary_pool['middle'][quality])

def generate_versus_commentary(beat, score):
    """Generate commentary for versus promos."""
    # Commentary pool by score range for versus promos
    commentary_pool = {
        'great': [
    ...
    # Select a random commentary line from the appropriate pool
    return random.choice(commentary_pool[quality]) 